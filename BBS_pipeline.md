...
### Packages used for data wrangling only
- dplyr
- reshape2
- stringr
### Packages used for producing results
- ape
- phytools
- FD
- picante

## List of input and outputs at each step
...
- Output: the pooled matrix and covariate table as well as the phylogenetic distance matrix and functional distance matrix; pooled for 2001-2011 and with a separate matrix for all birds and for residents only
...
- Input: Outputs from previous step, and `BBS_SpatialJoin.csv` file.
...
The location of each of the fifty stops along each BBS route was assigned to the hydrologic units (HUC4, HUC8, and HUC12) in which it is located. Each BBS route was assigned to hydrologic units if any stops along that route were contained in those polygons. Therefore, a single BBS route could be counted toward the diversity of multiple hydrologic units at a given level.
