...
# QDR/Aquaxterra/created 07Dec2016/modified 09dec2016
...
# Added 09 Dec: By route. Mean from 2001-2011

bbs_div_byroute <- read.csv('C:/Users/Q/Dropbox/projects/aquaxterra/bbs_div_byroute.csv', stringsAsFactors = FALSE)
...
# Comparison of different types of diversity, and their distributions.
png('figs/bbs_diversity_pairs_byroute.png', height=8, width=8, res=300, units='in')
ggpairs(bbs_div_byroute %>% select(richness, FRic, PD), # Looks like phylogenetic and taxonomic are much more highly correlated, while functional is different.
        diag=list(continuous=wrap('barDiag', bins=20)), columnLabels = c('TD','FD','PD')) + theme_bw()
dev.off()
...
for (i in vars_to_plot) {
  map_i <- ggplot(huc4_fort, aes_string(x='long', y='lat', group='group', fill=i)) +
    ...
    theme_bw() + 
    theme(axis.text = element_blank(), axis.ticks = element_blank(), axis.title = element_blank(), panel.grid = element_blank(), panel.background = element_rect(color = 'black', fill = 'black'), panel.border = element_blank(), plot.background = element_rect(fill = 'black'), legend.position = c(0.13,0.1), legend.direction = 'horizontal', legend.title = element_blank())
  ggsave(filename = paste0('C:/Users/Q/Dropbox/projects/aquaxterra/hucfigures/huc4/huc4_20012011_byroute_', i, '_map.png'), plot = map_i, height = 6, width = 9, dpi = 400)
  print(i)
}