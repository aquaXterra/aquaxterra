...
nhd_huc_data <- lapply(nhd_size, function(x) {
  if (is.null(length(x)) | length(x) == 0) return(NA)
  ids <- correct_ids(x)
...
nhd_huc_summary_means <- lapply(nhd_huc_data, function(x) {
  # size is length or area, depending on river vs. lake
  if (is.null(x)) return(data.frame(mean_size = NA, median_size = NA, sd_size = NA, n = NA))
  x %>%
    filter(include == 'yes') %>%
