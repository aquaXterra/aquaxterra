...
# State boundaries
states <- read.csv('/mnt/research/aquaxterra/DATA/albers_projected_data/state_borders/states_albers.csv', stringsAsFactors = FALSE)
...
huc4 <- readOGR(dsn = '/mnt/research/aquaxterra/DATA/albers_projected_data/HUC', layer = 'HU4_CONUS_Alb')

# Load data file
bbs_div4 <- read.csv('/mnt/research/aquaxterra/DATA/albers_projected_data/BBS_Diversity/bbs_div_huc4_mean.csv', header = TRUE)

rast4 <- read.csv('/mnt/research/aquaxterra/DATA/HUCed_data/raster/HUC4summarized.csv', header = TRUE)
...
huc4@data <- huc4@data %>%
  ...
  left_join(rast4[, c(1, grep('^mean_gpp\\_\\d+', names(rast4)))], by = 'HUC4')
...
get_stats <- function(data, index) {
  newdat <- gather(data[i, grep('^mean_gpp\\_\\d+', names(data))], key = year, value = mean_gpp)
  newdat$year <- as.numeric(gsub('^mean_gpp_', '', newdat$year))
  newdat$mean_gpp <- as.numeric(newdat$mean_gpp)/1000
  mean <- mean(newdat$mean_gpp, na.rm = TRUE)
  # scale to get relative increase
  newdat$mean_gpp <- scale(newdat$mean_gpp)
  mod <- lm(newdat$mean_gpp~newdat$year, data = newdat)
  slope <- summary(mod)$coefficients[[2]]
  sig <- summary(mod)$coefficients[[8]] <= 0.05
  return(c(slope, sig, mean))
}

huc4@data$gpp_trend <- NA
huc4@data$gpp_trend_dir <- NA
huc4@data$gpp_mean <- NA
huc4@data$gpp_sig <- NA
for (i in 1:nrow(huc4@data)) {
  stats <- get_stats(data = huc4@data, i)
  huc4@data$gpp_trend[i] <- stats[1]
  if(stats[1] > 0) {
    huc4@data$gpp_trend_dir[i] <- '\U2191'
  } 
  if(stats[1] < 0) {
    huc4@data$gpp_trend_dir[i] <- '\U2193'
  } 
  else{
    huc4@data$gpp_trend_dir[i] <- '\U0020'
  }
  huc4_pts@data$gpp_sig[i] <- stats[2]
  huc4@data$gpp_mean[i] <- stats[3]
}
huc4_pts@data$gpp_trend <- NA
huc4_pts@data$gpp_trend_dir <- NA
huc4_pts@data$gpp_mean <- NA
huc4_pts@data$gpp_sig <- NA
for (i in 1:nrow(huc4_pts@data)) {
  stats <- get_stats(data = huc4_pts@data, i)
  huc4_pts@data$gpp_trend[i] <- stats[1]
  if(stats[1] > 0) {
    huc4_pts@data$gpp_trend_dir[i] <- '\U2191'
  } 
  if(stats[1] < 0) {
    huc4_pts@data$gpp_trend_dir[i] <- '\U2193'
  } 
  if(stats[1] == 0){
    huc4_pts@data$gpp_trend_dir[i] <- '\U0020'
  }
  huc4_pts@data$gpp_sig[i] <- stats[2]
  huc4_pts@data$gpp_mean[i] <- stats[3]
}
...
the_map <- ggplot(huc4_fort) +
    geom_polygon(aes_string(x = 'long', y = 'lat', group = 'group', fill = 'gpp_mean')) +
    geom_path(aes_string(x = 'long', y = 'lat', group = 'group'), color = 'white', size = 0.25) +
    geom_text(data = huc4_pts_fort, aes(x = long, y = lat, label = sprintf(gpp_trend_dir), size = abs(gpp_trend), color = as.factor(gpp_sig))) +
    geom_path(data = states, aes(x = long, y = lat, group = group), color = 'gray20') +
    scale_fill_gradientn(colors = browntobluecolors, name = 'Mean GPP') +
    scale_size_continuous(name = 'GPP Change (stdev) / Year') +
...
ggsave(filename = file.path('/mnt/research/aquaxterra/FIGS/', 'huc4_summary_gpp_trend_map.png'), plot = the_map, height = 6, width = 9, dpi = 400)
